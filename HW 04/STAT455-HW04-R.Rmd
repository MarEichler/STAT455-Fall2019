...
C1 <- exp(confint(logit)[2,])

#PROFILE LIKELIHOOD adj
Count.adj <- Count+1
y.data.adj <- c(rep(1, Count.adj[1,1]), rep(0, Count.adj[1,2]), rep(1, Count.adj[2,1]), rep(0, Count.adj[2,2]))
x.data.adj <- c(rep(1, Count.adj[1,1]), rep(1, Count.adj[1,2]), rep(0, Count.adj[2,1]), rep(0, Count.adj[2,2]))
df.data.adj <- data.frame("Y"=y.data.adj, "X"=x.data.adj)
logit.adj <- glm(Y ~ X, data=df.data.adj, family=binomial)
C2 <- exp(confint(logit.adj)[2,])



names <- c("Woolf (Wald)", "Cornfield Exact", "Profile Likelihood", "Profe Likelihood, counts+1")
results<- cbind( names, decimal(rbind(A, B, C1, C2), dec))
