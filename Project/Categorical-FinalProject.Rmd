...
Looking just at the the totals of the 15 datasets and individual may think that the violence peaked in 2003-2004 and the total of victims is $\textcolor{red}{\text{3000?}}$ However, just looking at the trends amoung the totals of the 15 datasets does not give the whole picture.  
...
```{r descriptive-stats, fig.height=5.6, fig.cap="Capture-Recapture Frequency Statistics"}
...
The below describes three possible models.  The $\log(\hat{N})$ is based on the grand mean $\mu$ and effects based on our 4 systems (which are represented by $\lambda$).  The subscripts of $\lambda$ denote whether the effect is coming from a single system or the overlap between two systems (note we did not look at models that more than two-way interactions).  
